...
internal sealed class SslEventPump : IDisposable
{
    ...
    private void AcceptConnections()
    {
        while (true)
        {
            ...
            // The session takes ownership of the fd (SafeSocketHandle) and configures
            // SSL_set_fd + server accept state internally via SetContext.
            var socketHandle = new SafeSocketHandle((IntPtr)clientFd, ownsHandle: true);
