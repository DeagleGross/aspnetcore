...
/// <summary>
/// Connection context for an established DirectSsl connection.
///
/// After the handshake completes:
/// - Owns the <see cref="SslConnectionState"/> (which wraps the TlsSocketSession)
/// - Uses the assigned <see cref="SslEventPump"/> for all I/O (read/write via epoll)
/// - TLS operations happen on the pump's dedicated thread
...
internal sealed class DirectSslConnection : TransportConnection
{
    ...
    public override async ValueTask DisposeAsync()
    {
        ...
        // 6. Graceful SSL and socket shutdown. Disposing the connection state sends
